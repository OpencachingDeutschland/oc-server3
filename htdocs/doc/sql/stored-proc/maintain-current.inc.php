				...
				FOR EACH ROW
					BEGIN
						...
						IF ISNULL(@XMLSYNC) OR @XMLSYNC!=1 THEN
							...
							IF OLD.`needs_maintenance`!=NEW.`needs_maintenance` OR
							   OLD.`listing_outdated`!=NEW.`listing_outdated` OR
							   OLD.`wp_gc_maintained`!=NEW.`wp_gc_maintained` THEN
	...
	sql("CREATE TRIGGER `cachesAfterUpdate` AFTER UPDATE ON `caches`
				FOR EACH ROW
					BEGIN
						...
						IF ROUND(NEW.`longitude`,6) != ROUND(OLD.`longitude`,6)
						   OR ROUND(NEW.`latitude`,6) != ROUND(OLD.`latitude`,6) THEN
