	...
	*/
	...
	function sql_internal($dblink, $sql)
	{
		...
		while ($nextarg !== false)
		{
			// & escaped?
			$escapesCount = 0;
			while ((($nextarg - $escapesCount - 1) > 0) && (substr($sql, $nextarg - $escapesCount - 1, 1) == '\\')) $escapesCount++;
			if (($escapesCount % 2) == 1)
				$nextarg++;
			else
			{
				...
				else
				{
					...
					else if (isset($db['temptables'][$arg]))
					{
						if (substr($sql, $nextarg - 1, 1) != '`')
							$filtered_sql .= '`';
		...
		{
			$escapesCount = 0;
			while ((($nextarg - $escapesCount - 1) > 0) && (substr($filtered_sql, $nextarg - $escapesCount - 1, 1) == '\\')) $escapesCount++;
			if (($escapesCount % 2) == 0)
			{
				// strip escapes of &
	...
	function sql_escape($value)
	{
		...
		// establish db connection
		if ($db['connected'] != true) sql_connect();
