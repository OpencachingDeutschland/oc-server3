...
if (isset($_REQUEST['submit']) && $_REQUEST['submit'] && $login->userid > 0) {
    ...
    if (isset($_REQUEST['opt_maxcaches'])) {
        $useroptions->setOptValue(
            ...
            $_REQUEST['opt_maxcaches'] == 0 ? 0 : min(
                max(
                    round($_REQUEST['opt_maxcaches'] + 0),
                    $opt['map']['min_maxrecords']
                ),
...
if (isset($_REQUEST['lat']) &&
    ...
    $rsCoords = sql(
        "SELECT `gmLat`, `gmLon`, `gmZoom` FROM `countries_options` WHERE `country`='&1'",
...
$rsCacheType = sql(
    "SELECT `cache_type`.`id`, IFNULL(`sys_trans_text`.`text`, `cache_type`.`short2`) AS `text`
    FROM `cache_type`
    LEFT JOIN `sys_trans_text`
        ON `cache_type`.`short2_trans_id`=`sys_trans_text`.`trans_id`
...
$rsCacheSize = sql(
    "SELECT `cache_size`.`id`, IFNULL(`sys_trans_text`.`text`, `cache_size`.`name`) AS `text`
    FROM `cache_size`
    LEFT JOIN `sys_trans_text`
        ON `cache_size`.`trans_id`=`sys_trans_text`.`trans_id`
        AND `sys_trans_text`.`lang`='&1'
...
/* assign attributes */
$tpl->assign('aAttributes', OcLib2\attribute::getSelectableAttributesListArray());
...
function output_cachexml($sWaypoint)
{
    ...
    $rsCache = sql_slave(
        "SELECT `caches`.`cache_id`, `caches`.`name`, `caches`.`wp_oc`, `caches`.`cache_id`, `caches`.`type`,
    ...
    $nFoundCount = sql_value_slave(
        "SELECT COUNT(*) FROM `cache_logs` WHERE `user_id`='&1' AND `cache_id`='&2' AND `type`=1",
        0,
        $login->userid,
        $rCache['cache_id']
    );
    if ($nFoundCount == 0) {
        $nNotFoundCount = sql_value_slave(
            "SELECT COUNT(*) FROM `cache_logs`
            WHERE `user_id`='&1' AND `cache_id`='&2' AND `type`=2",
            0,
            $login->userid,
            $rCache['cache_id']
        );
    ...
    if ($rCache['type'] == 6) {
        $nAttendedCount = sql_value_slave(
            "SELECT COUNT(*)
            FROM `cache_logs`
            WHERE `user_id`='&1'
            AND `cache_id`='&2'
...
function output_searchresult(
    ...
    sql(
        "UPDATE `map2_result`
        SET `request_counter`=`request_counter`+1, `date_lastqueried`=NOW()
        WHERE `result_id`='&1'",
        $nResultId
    );

    // execute query and return search result
    $nRecordCount = sql_value_slave(
        "SELECT COUNT(*)
        FROM `map2_data`
        INNER JOIN `caches` ON `map2_data`.`cache_id`=`caches`.`cache_id`
        WHERE `map2_data`.`result_id`='&1'
        AND `caches`.`longitude`>'&2'
        AND `caches`.`longitude`<'&3'
        AND `caches`.`latitude`>'&4'
        AND `caches`.`latitude`<'&5'",
        0,
        $nResultId,
        $nLon1,
        $nLon2,
    ...
    if ($login->userid > 0) {
        $user_maxrecords =
            sql_value(
                "SELECT option_value FROM user_options WHERE user_id='&1' AND option_id=8",
                $opt['map']['maxrecords'] + 0,
                $login->userid
            );
