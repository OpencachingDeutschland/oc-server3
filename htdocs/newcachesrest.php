...
if (!$tpl->is_cached()) {
    ...
    $rsNewCaches = sql_slave(
        "SELECT IFNULL(`sys_trans_text`.`text`,
         ...
         INNER JOIN `cache_type`
           ON `caches`.`type`=`cache_type`.`id`
         INNER JOIN &cachelist
           ON &cachelist.`cache_id`=`caches`.`cache_id`
         ...
         LEFT JOIN `caches_attributes` `ca`
           ...
           AND `ca`.`attrib_id`=6
         WHERE `status` = 1
         ORDER BY `country_name`, `caches`.`date_created` DESC
         LIMIT 200",
        $opt['template']['locale']
    );
