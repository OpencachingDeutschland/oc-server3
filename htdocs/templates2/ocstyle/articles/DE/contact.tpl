{***************************************************************************
* You can find the license in the docs directory
***************************************************************************}
    <div class="content2-pagetitle">
        <img src="resource2/{$opt.template.style}/images/misc/32x32-tools.png" style="margin-right: 10px;" width="32" height="32" alt="" />Kontaktm&ouml;glichkeiten
    </div>
    <div class="content-txtbox-noshade" style="padding-right: 25px;">

<table class="table">

    <tr>
        <td class="header-small"><b>Allgemeine Fragen zu Opencaching.de und zum Thema Geocaching</b></td>
    </tr>
    <tr><td class="spacer"></td></tr>
    <tr>
        <td>
            <p>Für Fragen rund um Opencaching und zum Thema Geocaching ist das <a href="http://wiki.opencaching.de">Opencaching-Wiki</a> eine gute Anlaufstelle. Weitere Informationen zum Geocaching gibt es auf <a href="http://www.geocaching-info.de">www.geocaching-info.de</a>.</p>
            <p>Wenn du ein spezielles Problem hast und darauf keine Antwort findest,
            kannst du dir unter <a href="http://forum.opencaching.de/">forum.opencaching.de</a>
            ein passendes Forum raussuchen und dich dort erkundigen.</p>
        </td>
    </tr>

    <tr><td class="spacer"></td></tr>
    <tr>
        <td class="header-small"><b>Bedienung der Website, Anregungen und Kritik</b></td>
    </tr>
    <tr><td class="spacer"></td></tr>
    <tr>
        <td>
            Hierfür gibt es ein eigenes Unterforum auf
            <a href="http://forum.opencaching.de/index.php?board=33.0">forum.opencaching.de</a>.
            Dort findest du auch weitere Informationen, falls du in unserem Team mitmachen möchtest.
        </td>
    </tr>

    <tr><td class="spacer"></td></tr>
    <tr>
        <td class="header-small"><b>Sonstiges</b></td>
    </tr>
    <tr><td class="spacer"></td></tr>
    <tr>
        <td>
            <p>Sollten die oben genannten Möglichkeiten nicht ausreichen oder die Betreiber von <i>opencaching.de</i> direkt
            kontaktiert werden, kannst du auch eine Email an <a href="mailto:{$opt.mail.contact}">{$opt.mail.contact}</a> schreiben.</p>
            <p>Bitte werde nicht ungeduldig wenn nicht sofort eine Antwort kommt, <i>opencaching.de</i> wird von Freiwilligen
            betreut, die leider nicht immer und sofort zur Verfügung stehen können.</p>
        </td>
    </tr>
</table>

<br />
</div>
