...
var tip_general_nm = "{t}Select <i>needs maintenance</i> if the geocache was in poor condition at the<br />specified date and in urgent need of maintenance. Please explain why.{/t}<br /><div style='height:0.3em'></div>{t}Select <i>ok</i> if you have found the cache and everything is in acceptable<br />condition.{/t}";
var tip_general_lo = "{if $has_gc_listing}{t}Select <i>is outdated</i> if the geocache search is hampered by outdated information in<br />the description, e.g. the location has severely changed or the description lacks<br />important information which has been added at another geocaching website.<br />Please give details in your log.{/t}{else}{t}Select <i>is outdated</i> if the geocache search is hampered by outdated information<br />in the description, e.g. because the location has severely changed. Please give<br />details in your log.{/t}{/if}<br /><div style='height:0.3em'></div>{t}Select <i>up to date</i> if you have checked the complete description &ndash; from the<br />container size to encoded hints &ndash; and can confirm that everything is up-to-date.{/t}";
