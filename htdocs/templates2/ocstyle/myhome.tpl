...
{if $allpics === false}
    ...
    <div class="content2-container bg-blue02" style="margin-top:20px;">
        <p class="content-title-noshade-size3">
            <img src="resource2/{$opt.template.style}/images/description/22x22-logs.png" width="22" height="22"
                 style="margin-right: 10px;"/>&nbsp;
            {t 1=$found}Finds: %1{/t} &nbsp;
            {if $logs|@count > 0}<span class="content-title-link">[<a
                        href="search.php?showresult=1&amp;expert=0&amp;output=HTML&amp;sort=bymylastlog&amp;finderid={$login.userid}&amp;searchbyfinder=&amp;f_inactive=0&amp;logtype=1,7&amp;calledbysearch=0">{t}Geocaches found{/t}</a>]&nbsp; [<a
                        href="search.php?showresult=1&amp;expert=0&amp;output=HTML&amp;sort=bymylastlog&amp;finderid={$login.userid}&amp;searchbyfinder=&amp;f_inactive=0&amp;logtype=0&amp;calledbysearch=0">{t}Geocaches logged{/t}</a>]
                </span>{/if}
        </p>
    </div>
    {* Ocprop: (find|us|own)erid=([0-9]+) *}
    <p style="line-height: 1.6em;"><b>{t}Your latest log entries{/t}{if $morelogs} </b>(
        <a class="systemlink" href="ownlogs.php">{t}more{/t}</a>
                                                                                       ){t}#colonspace#{/t}
                                                                                       :{else}{t}#colonspace#{/t}
                                                                                       :</b>{/if}</p>
    <table class="table">
                ...
                <td>
                    <nobr>
                        ...
                        {if $logItem.oc_team_comment}<img
                            src="resource2/{$opt.template.style}/images/oclogo/oc-team-comment.png" alt="OC-Team"
                            title="{t}OC team comment{/t}" />{/if}
                    </nobr>
                </td>
...
<div class="content2-container bg-blue02" style="margin-top:20px;">
    <p class="content-title-noshade-size3">
        <img src="resource2/{$opt.template.style}/images/misc/32x32-pictures.gif" width="24" height="24"
             style="margin-right: 10px;"/>&nbsp;
        {t 1=$total_pictures}Log pictures: %1{/t} &nbsp;
        {if $pictures|@count > 0 && $allpics === false}<span class="content-title-link">[<a
                    href="myhome.php?allpics=ownlogs">{t}Show all{/t}</a>]</span>{/if}
    </p>
</div>
...
<div class="content2-container bg-blue02" id="mycaches" style="margin-top:5px">
    <p class="content-title-noshade-size3">
        <img src="resource2/{$opt.template.style}/images/cacheicon/22x20-traditional.png" width="22" height="20"
             style="margin-right: 10px;"/>&nbsp;
        {t 1=$hidden}Geocaches hidden: %1{/t} &nbsp;
        {* Ocprop: (find|us|own)erid=([0-9]+) *}
        {if $caches|@count > 0}<span class="content-title-link">[
            <a
            href="search.php?showresult=1&amp;expert=0&amp;output=HTML&amp;sort=bycreated&amp;ownerid={$login.userid}&amp;searchbyowner=&amp;f_inactive=0&calledbysearch=0">{t}Show details{/t}</a>{if $active < $hidden}]&nbsp; [
...
<table class="table" style="max-width:97%">
    ...
    {else}
        ...
        <tr>
            <td colspan="4">
                <span style="line-height:2em"><b>{t}Your geocaches hidden{/t}</b><span id="toggle_archived_option"
                                                                                       style="display:none">{if $archived>0} (




                            <a href="javascript:toggle_archived()" style="outline:none"><span
                                        id="hide_archived">{t}hide archived{/t}</span><span id="show_archived"
                                                                                            style="display:none">{t}show archived{/t}</span></a>




){/if}</span>{t}#colonspace#{/t}:</span>
            </td>
            <td style="text-align:right">
                <span style="line-height:2em"><img src="images/rating-star.gif" width="17" height="16"
                                                   title="{t}with recommendation{/t}"/></span>
            </td>
            <td style="text-align:right">
                <span style="line-height:2em"><img src="resource2/{$opt.template.style}/images/log/16x16-found.png"
                                                   alt="{t}Found{/t}" title="{t}Found{/t}"/></span>
            </td>
            <td style="text-align:left"><span style="line-height:2em; text-decoration: underline;"><b><a
                                href="myhome.php?sort=lastLog">{t}Last log{/t}</a></b></span></td>
        </tr>
        {foreach from=$caches item=cacheItem}
            {if $dotfill == ''}
            ...
            <tr {if $cacheItem.status>2}name="row_archived"{/if}>
                ...
                <td class="{$listcolor}" style="text-align:center">
                    <nobr>{$cacheItem.date_hidden|date_format:$opt.format.datelong}&nbsp;</nobr>
                </td>
                <td class="{$listcolor}">{include file="res_cachestatus.tpl" status=$cacheItem.status}</td>
                <td class="{$listcolor}"
                    style="{if strlen($cacheItem.name) < 45}white-space:nowrap;{/if}min-width:300px;max-width:{if $dotfill==''}400{else}300{/if}px;overflow:hidden;">
                    <a href="viewcache.php?wp={$cacheItem.wp_oc}">{$cacheItem.name|escape}</a>{if strlen($cacheItem.name) < 45} {include file="res_logflags.tpl" logItem=$cacheItem lfSpace=true} &nbsp;&nbsp;
                    <span style="color:#b0b0b0">{$dotfill}</span>{/if}</td>
                <td class="{$listcolor}" style="text-align:right;" align="right">
                    <nobr>{if $cacheItem.toprating>0}{$cacheItem.toprating}{/if}</nobr>
                </td>
                <td class="{$listcolor}" style="text-align:right;" align="right">
                    <nobr>{if $cacheItem.found>0}{$cacheItem.found}{/if}</nobr>
                </td>
                <td class="{$listcolor}" style="text-align:right;" align="right">
                    <nobr>
                        <a href="viewcache.php?cacheid={$cacheItem.cache_id}#logentries">{$cacheItem.lastlog|date_format:$opt.format.date}</a>&nbsp; {include file="res_logtype.tpl" type=$cacheItem.lastlog_type}
                    </nobr>
                </td>
            </tr>
        {/foreach}
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                {foreach from=$notpublished item=notpublishedItem}
                    <div class="col-2 p-0 unpublished-container">
                        <div class="oc-waypoint-code">
                            {$notpublishedItem.wp_oc}
                        </div>
                        <div class="p-1 oc-waypoint-name">
                        ...
                        <div class="p-1 oc-waypoint-edit">
                            <button name="ShowLog"
