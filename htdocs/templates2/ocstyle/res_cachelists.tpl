					...
					{foreach from=$cachelists item=cachelist}
						...
						<tr class="cachelistitem">
							...
							<td class="{$listcolor}" style="text-align:right" >
								{if $show_edit}[<a href="mylists.php?edit={$cachelist.id}">{t}edit{/t}</a>]&nbsp;[<a href="javascript:if(confirm('{t 1=$cachelist.name|escapejs}Do you really want to delete the list \'%1\'?{/t}'))location.href='mylists.php?delete={$cachelist.id}'">{t}delete{/t}</a>]{/if}
