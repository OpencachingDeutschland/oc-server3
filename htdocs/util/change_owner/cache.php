 ...
 /***************************************************************************
    ...
    ***************************************************************************/
    ...
    if ($action == 'changeowner') {
        ...
        $rsCache = sql("SELECT caches.cache_id, user.user_id FROM caches INNER JOIN user ON caches.user_id=user.user_id WHERE caches.cache_id='&1'", $cacheid);
        ...
        $rsCache = sql("SELECT caches.cache_id, caches.wp_oc, caches.name, user.username FROM caches INNER JOIN user ON caches.user_id=user.user_id WHERE caches.cache_id='&1'", $cacheid);
        $rCache = sql_fetch_assoc($rsCache);
        sql_free_result($rsCache);
        ?>
<html>
	<body>
        ...
        ?>" />
			<table>
				<tr>
					<td>Wegpunkt</td>
					<td><?php echo htmlspecialchars($rCache['wp_oc']);
        ?></td>
				</tr>
				<tr>
					<td>Name</td>
					<td><?php echo htmlspecialchars($rCache['name']);
        ?></td>
				</tr>
				<tr>
					<td>Benutzer</td>
					<td><?php echo htmlspecialchars($rCache['username']);
        ?></td>
				</tr>
				<tr><td colspan="2">&nbsp;</td></tr>
				<tr>
					<td>Neuer Benutzer</td>
					<td><?php echo htmlspecialchars($rUser['username']);
        ?></td>
				</tr>
				<tr><td colspan="2">&nbsp;</td></tr>
				<tr>
