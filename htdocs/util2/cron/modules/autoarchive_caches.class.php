			    ...
			    ($rCache['listing_age'] > 184 &&
			     (sql_value("SELECT DATEDIFF(NOW(),'&1')", 0, $rCache['disable_date']) > 366 ||
			      (!$rCache['seasonal_cache'] &&
			       ...
			       &&
			       sql_value("SELECT `type` FROM `cache_logs` WHERE `cache_id`='&1' ORDER BY `order_date` DESC, `date_created` DESC, `id` DESC LIMIT 1", '', $rCache['cache_id']) == cachelog::LOGTYPE_DISABLED
			      )
			     )
			    )
			   )
