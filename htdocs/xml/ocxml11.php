...
/* begin parameter reading */
    ...
    if (isset($_REQUEST['sessionid'])) {
        ...
        } elseif (isset($_REQUEST['lat']) || isset($_REQUEST['lon']) || isset($_REQUEST['distance'])) {
        ...
        } elseif (isset($_REQUEST['cacheid']) || isset($_REQUEST['wp']) || isset($_REQUEST['uuid'])) {
            $selection['type'] = 3;
...
//	echo '<tr><td>sql-start</td><td>sql-count</td><td>count</td><td>begin</td><td>end</td></tr>';
    for ($i = 0; $i < 6; $i++) {
        ...
        } elseif (($startat >= $recordnr[$i]) && ($startat < $recordnr[$i + 1])) {
            $limits[$i] = array('start' => $startat - $recordnr[$i], 'count' => $recordnr[$i + 1] - $startat);
            //$limits[$i] = array('start' => 'b', 'count' => 'b');
        } elseif (($startat + 500 >= $recordnr[$i]) && ($startat + 500 < $recordnr[$i + 1])) {
        ...
        } elseif (($startat < $recordnr[$i]) && ($startat + 500 >= $recordnr[$i + 1])) {
            $limits[$i] = array('start' => 0, 'count' => $recordnr[$i + 1] - $recordnr[$i]);
