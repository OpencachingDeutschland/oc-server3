...
// boot.js — OCmap map orchestrator
...
const mapRootEl = document.getElementById('mapRoot');
const initLat  = mapRootEl?.dataset.initLat  ? parseFloat(mapRootEl.dataset.initLat)  : null;
const initLon  = mapRootEl?.dataset.initLon  ? parseFloat(mapRootEl.dataset.initLon)  : null;
const initZoom = mapRootEl?.dataset.initZoom ? parseInt(mapRootEl.dataset.initZoom, 10) : null;
const uniCacheWP = [];
...
if (initLat !== null && initLon !== null && initZoom !== null) {
  mapRoot.setView([initLat, initLon], initZoom);
} else {
...
let liveEnabled = true; // Enabled by default now
...
//-------------------------
// fetchOCByBbox() — calls /api/caches/live backend
...
async function fetchOCByBbox(s, w, n, e) {
  ...
  try {
    const res = await fetch('/api/caches/live?' + params);
...
export async function handleWPs() {
  if (!uniCacheWP.length) {
    mapRoot.setView([initLat ?? 51.5, initLon ?? 10.0], initZoom ?? 6);
    if (!mapRoot.hasLayer(liveMap)) {
      liveMap.addTo(mapRoot);
    } else {
      fetchAndShowLiveMarkers();
    ...
    if (uniCacheWP.length === 1) {
      const wp = uniCacheWP[0];
      mapRoot.setView([wp.lat, wp.lon], initZoom ?? 14);
    } else {
...
async function handleMarkerClick(marker) {
  ...
  try {
    const res = await fetch(`/api/caches/waypoints?wp=${referenceCode}`);
    if (!res.ok) return;
    const data = await res.json();
...
export function createMarker(p) {
  ...
  const linkHtml  = `<a href="/cache/${p.referenceCode}" title="${p.name}">${p.referenceCode} ${shortName}</a>`;
...
export async function boot(staticWPs = []) {
  if (staticWPs.length) {
    uniCacheWP.length = 0;
    uniCacheWP.push(...staticWPs);
    liveEnabled = false;
  }
