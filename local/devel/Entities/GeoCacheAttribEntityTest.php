...
class GeoCacheAttribEntityTest extends AbstractModuleTest
{
    public function testEntity(): void
    {
        ...
        $entity->groupId = mt_rand(0, 100);
        $entity->selectable = mt_rand(0, 100);
        $entity->category = mt_rand(0, 100);
        $entity->searchDefault = mt_rand(0, 100);
        $entity->default = mt_rand(0, 100);
        $entity->iconLarge = md5(time());
        $entity->iconNo = md5(time());
        $entity->iconUndef = md5(time());
        $entity->htmlDesc = md5(time());
        $entity->htmlDescTransId = mt_rand(0, 100);
        $entity->hidden = mt_rand(0, 100);
        $entity->gcId = mt_rand(0, 100);
        $entity->gcInc = mt_rand(0, 100);
        $entity->gcName = md5(time());
