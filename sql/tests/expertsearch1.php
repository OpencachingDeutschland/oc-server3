 ...
 */
sql(
    'CREATE TEMPORARY TABLE remove_caches ENGINE=MEMORY SELECT DISTINCT result_caches.cache_id cache_id FROM result_caches, cache_logs WHERE result_caches.cache_id=cache_logs.cache_id AND cache_logs.user_id=101254'
);
 ...
 */
sql(
    'CREATE TEMPORARY TABLE remove_caches ENGINE=MEMORY SELECT DISTINCT result_caches.cache_id cache_id FROM result_caches, cache_logs WHERE result_caches.cache_id=cache_logs.cache_id AND cache_logs.user_id=101301'
);
sql('ALTER TABLE remove_caches ADD PRIMARY KEY (cache_id)');
sql('DELETE FROM result_caches WHERE cache_id IN (SELECT cache_id FROM remove_caches)');
sql('DROP TABLE remove_caches');
 ...
 */
sql(
    'CREATE TEMPORARY TABLE remove_caches ENGINE=MEMORY SELECT result_caches.cache_id cache_id FROM result_caches, caches WHERE result_caches.cache_id=caches.cache_id AND caches.status!=1'
);
sql('ALTER TABLE remove_caches ADD PRIMARY KEY (cache_id)');
sql('DELETE FROM result_caches WHERE cache_id IN (SELECT cache_id FROM remove_caches)');
sql('DROP TABLE remove_caches');
 ...
 */
sql(
    'CREATE TEMPORARY TABLE remove_caches ENGINE=MEMORY SELECT result_caches.cache_id cache_id FROM result_caches, caches WHERE result_caches.cache_id=caches.cache_id AND caches.search_time>2'
);
sql('ALTER TABLE remove_caches ADD PRIMARY KEY (cache_id)');
sql('DELETE FROM result_caches WHERE cache_id IN (SELECT cache_id FROM remove_caches)');
sql('DROP TABLE remove_caches');
